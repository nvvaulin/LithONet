...
from seismic.models.classification.transform import get_augmentations, get_preprocessing
...
def train_net(net, train_loader, val_loader, device, optimizer, model_save_path=None, epochs=5,
              best_val_accuracy=None, init_epoch=0, criterion=None, writer=None, lr_scheduler=None):
    if not criterion:
        criterion = torch.nn.CrossEntropyLoss()

    best_val_accuracy = 0.0 if best_val_accuracy is None else best_val_accuracy
    best_net = None
    ...
    for epoch in tqdm(range(init_epoch, epochs + init_epoch), desc='epoch'):
        ...
        writer.add_scalar('Loss/train', running_loss / (i+1), global_step=iter)
        writer.add_scalar('Accuracy/train', train_stat['accuracy'], global_step=iter)
        writer.add_scalar('Precision/train', train_stat['precision'], global_step=iter)
        writer.add_scalar('Recall/train', train_stat['recall'], global_step=iter)

        if model_save_path:
            state = {'val_accuracy': val_stat['accuracy'], 'epoch': epoch, 'state_dict': net.state_dict(),
                     'optimizer': optimizer.state_dict()}
            if val_stat['accuracy'] > best_val_accuracy:
                writer.add_scalar('Accuracy_best/val', val_stat['accuracy'], global_step=iter)
...
if __name__ == "__main__":
    ...
    net = train_net(net, train_loader, val_loader, device, optimizer,
                    ...
                    best_val_accuracy=val_accuracy, init_epoch=init_epoch, lr_scheduler=lr_scheduler)
